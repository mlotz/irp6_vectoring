...
def ToContact():
	...
	irpos.move_rel_to_cartesian_pose_with_contact( 20.0, Pose(Point(0.0, 0.0, 0.20), Quaternion(0.0, 0.0, 0.0, 1.0)),Wrench(Vector3(10.0, 10.0, 5.0), Vector3(0.0, 0.0, 0.0)))
	irpos.move_rel_to_cartesian_pose(2.0, Pose(Point(0.0, 0.0, -0.01), Quaternion(0.0, 0.0, 0.0, 1.0)))

	irpos.move_rel_to_cartesian_pose_with_contact( 20.0, Pose(Point(0.20, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)),Wrench(Vector3(4.0, 4.0, 0.0), Vector3(0.0, 0.0, 0.0)))
...
def TRacking():
	...
	print "Irp6p: Behavior: TRacking - Starting."
	...
	irpos.move_rel_to_cartesian_pose(6.0, Pose(Point(0.0, 0.0, 0.05), Quaternion(0.0, 0.0, 0.0, 1.0)))
	irpos.move_rel_to_cartesian_pose(6.0, Pose(Point(-0.1, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	time.sleep(1.0)
	#irpos.move_rel_to_cartesian_pose(8.0, Pose(Point(0.2, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	#time.sleep(1.0)
	#irpos.move_rel_to_cartesian_pose(8.0, Pose(Point(-0.2, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	#time.sleep(1.0)
	#for iter in range(1,4):
	#	irpos.seamless_move_rel_to_cartesian_pose(2.0, Pose(Point(0.05, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	...
	#	irpos.seamless_move_rel_to_cartesian_pose(0.04, Pose(Point(0.001, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	...
	for iter in range(0,719):
		...
		#irpos.seamless_move_rel_to_cartesian_pose(0.01, Pose(Point(np.sin(angle)*step, np.cos(angle)*step, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	...
	#	irpos.seamless_move_rel_to_cartesian_pose(0.002, Pose(Point(0.00005, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	#time.sleep(3.0)
	for iter in range(0,719):
		setMarker(iter,0.0)
		time.sleep(0.005)
	angle = 0;
	irpos.move_rel_to_cartesian_pose(4.0, Pose(Point(0.0, -0.1, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	irpos.on_pose_int()
	time.sleep(1.0)
	for iter in range(0,719):
		setMarker(iter,1.0)
		angle = np.pi/2* sin(iter * (np.pi/360))
		#print(str(angle))
		irpos.seamless_move_rel_to_cartesian_pose(0.04, Pose(Point(np.sin(angle)*step, np.cos(angle)*step, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	irpos.off_pose_int()
	time.sleep(1.0)
	...
	for iter in range(0,400):
		setMarker(iter,1.0)
		angle = np.pi-((np.pi/2)*floor(iter/100))
		#print(str(angle))
		irpos.seamless_move_rel_to_cartesian_pose(0.04, Pose(Point(np.sin(angle)*step, np.cos(angle)*step, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
	irpos.off_pose_int()
	for iter in range(0,400):
	...
	irpos.move_along_joint_trajectory(joint_trajectory)
	print "Irp6p: Behavior: TRacking - done."

def TRacking2():

	print "Irp6p: Behavior: TRacking2 - Starting."
	#rospy.wait_for_service('/irp6p_manager/switch_controller')
  	#conmanSwitchIrp6p = rospy.ServiceProxy('/irp6p_manager/switch_controller', SwitchController)
	#conmanSwitchIrp6p(['Irp6pmForceTransformation'], [], True)
	#irpos.off_pose_int()
	...
	#irpos.move_to_cartesian_pose(4.0,Pose(current.position,Quaternion(np.sin(-np.pi/4),np.sin(-np.pi/4),0.0,0.0)))
	time.sleep(1.0)
	...
	for iter in range(0,719):
		...
		if (zDelta > maxzDelta):
			maxzDelta = zDelta
		zV = -zDelta*0.5
		#print 'Delta Z:'+str(zDelta)+ 'so zV is:' +str(zV)
		if (zV>zVConstraint):
